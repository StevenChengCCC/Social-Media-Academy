  ...
  'en': {
    ...
    sections: {
      privacy: {
        ...
        steps: [
          'Profile Picture (Top Right) → Settings → General → Enable Restricted Mode (Must be enabled on each device).',
          'History → Pause watch/search history, or clear regularly to reduce personalized recommendations.',
      ...
      screenTime: {
        ...
        steps: [
          'Settings → General → Turn on Remind me to take a break, Bedtime reminder.',
          'Mobile can also track watch time: Profile → Your data in YouTube / Time watched.',
        ]
      },
      copyright: {
        h2: 'Copyright & Digital Literacy', pill1: 'Fair Use', pill2: 'Misinformation',
        steps: [
          'Copyright Basics: Do not upload content (music, movie clips, images) unless you created it or have permission. YouTube automatically scans for copyright infringement.',
          'Understanding Fair Use: Using small portions of copyrighted material for purposes like critique, commentary, news reporting, teaching, or parody may be protected by Fair Use.',
          'Spotting Misinformation: Check the source, look for multiple credible news reports, and be wary of highly emotional or sensationalized video titles and thumbnails.',
        ]
      },
      supervised: {
        h2: 'Supervised Experiences', pill1: 'YouTube Kids', pill2: 'Supervised Account',
        steps: [
          'Young children: Install YouTube Kids, parents set content level and watch time.',
          'School-age/Teens: Create a Supervised Account via Google Family Link, select content level (Explore / Explore More / Most of YouTube), control search and duration.',
        ]
      }
    },
    ytTitles: {
  ...
  'zh-CN': {
    ...
    sections: {
      privacy: {
        ...
        steps: [
          '右上角头像 → 设置 → 通用 → 开启 受限模式（每台设备都要开启）。',
          '历史记录 → 暂停观看/搜索历史，或定期清除，减少个性化推荐。',
      ...
      screenTime: {
        ...
        steps: [
          '设置 → 通用 → 打开 提醒我休息一下、就寝提醒。',
          '移动端还能统计观看时间：个人资料 → 您在 YouTube 中的数据 / 观看时间。',
        ]
      },
      copyright: {
        h2: '版权与数字素养', pill1: '合理使用', pill2: '错误信息',
        steps: [
          '版权基础：除非您是创作者或拥有许可，否则请勿上传内容（音乐、电影片段、图像）。YouTube 会自动扫描版权侵权。',
          '理解合理使用：出于评论、批评、新闻报道、教学或模仿等目的使用少量版权材料可能受到合理使用的保护。',
          '识别错误信息：检查来源，查找多个可靠的新闻报道，并警惕高度情绪化或耸人听闻的视频标题和缩略图。',
        ]
      },
      supervised: {
        h2: '监督体验', pill1: 'YouTube Kids', pill2: '监督账户',
        steps: [
          '年幼儿童：安装 YouTube Kids，家长设置内容等级与观看时间。',
          '学龄/青少年：通过 Google Family Link 创建 监督账户，选择内容级别（探索 / 探索更多 / 大部分 YouTube），控制搜索与时长。',
        ]
      }
    },
    ytTitles: {
